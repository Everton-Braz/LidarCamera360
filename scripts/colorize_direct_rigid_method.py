#!/usr/bin/env python3
"""Compatibility CLI for the unified direct rigid colorizer."""
...
from scripts.pipeline_auto_calibrator_and_colorizer import colorize_via_direct_rigid

def main(argv=None):
    parser = argparse.ArgumentParser(description="Colorize a LiDAR cloud with the calibrated rigid camera rig.")
    ...
    ply, pcd = map(Path, colorize_via_direct_rigid(args.dataset.resolve(), args.calib, fps=args.fps, dt_override=args.dt))
