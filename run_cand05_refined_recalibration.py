...
# -*- coding: utf-8 -*-
"""
Candidate 05: Refined Calibration Matching Point #13672 (Physical Frame Top-Left Corner)
========================================================================================
Ground-truth point from CloudCompare:
  P#13672: (6.561255, 0.088996, 1.092260) - Top-left physical frame corner on the wall.
Camera position in world:
  [-0.0037, -0.0003, -0.1872]
Physical height difference:
  Delta Z = 1.092 - (-0.187) = +1.279 m
Distance to wall:
  X = 6.56 m
Physical elevation angle:
  arctan(1.279 / 6.56) = +11.03 degrees

Camera image frame corner at 10.0s:
  u = 1902.0 px, v = 1918.0 px

Optimal calibration angles:
  Pitch: -10.93 deg
  Yaw:   -0.25 deg
  Roll:   0.00 deg

Projected Point #13672:
  u = 1902.1 px, v = 1918.1 px (error < 0.1 pixels!)
"""
...
OUT_DIR = os.path.join(DATASET_DIR, "calibration_cand05_refined")
...
def get_cand05_geometry(pitch_deg=-10.93, yaw_deg=-0.25):
...
def colorize_at_times(pts_world, trj, R_s0, R_s1, c_L, pairs_t_slam_t_video):
    ...
    for i, (t_slam_rel, v_time_sec) in enumerate(pairs_t_slam_t_video):
        ...
        # Candidate 05 transformation: P_cam = (P_l - c_L) @ R.T
        # Stream 0 (Right Lens)
        if img0 is not None:
...
def run():
    ...
    print("=" * 75)
    print(" CANDIDATE 05: REFINED CALIBRATION MATCHING POINT #13672")
    print("=" * 75)
    ...
    # 1. Optimal Cand05 Single Frame from Second 10
    print("\n[1/3] Generating Cand05 Optimal Single Frame at Video Time = 10.0s (Pitch = -10.93°, Yaw = -0.25°)...")
    R_s0, R_s1, c_L = get_cand05_geometry(pitch_deg=-10.93, yaw_deg=-0.25)
    ...
    out_opt = os.path.join(OUT_DIR, "cand05_refined_optimal_single_frame_10s.pcd")
    write_pcd(out_opt, pts_world, colors_opt)

    # 2. Fine Variations Around Optimal Pitch (-10.5°, -11.3°)
    for p_val in [-10.50, -11.30]:
        name = f"cand05_pitch_{p_val:+.2f}deg_single_frame_10s.pcd".replace("+", "p").replace("-", "m")
        print(f"\n[*] Generating Fine Sweep Variation: Pitch = {p_val:+.2f}°...")
        R0_v, R1_v, cL_v = get_cand05_geometry(pitch_deg=p_val, yaw_deg=-0.25)
    ...
    # 3. Full Trajectory Colorization with Cand05 Refined Extrinsics
    print("\n[3/3] Generating Cand05 Refined Full Trajectory Point Cloud (45 Keyframes)...")
