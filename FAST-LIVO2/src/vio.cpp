...
void VIOManager::initializeVIO()
{
  ...
  if(colmap_output_en)
  {
    ...
    else
    {
      colmap_width = 768;
      colmap_height = 1024;
      colmap_fx = 457.633379;
      colmap_fy = 457.828465;
      colmap_cx = 384.0;
      colmap_cy = 512.0;

      double fx_orig = fx / image_resize_factor;
      double fy_orig = fy / image_resize_factor;
      double cx_orig = cx / image_resize_factor;
      double cy_orig = cy / image_resize_factor;
      cv::Mat cvK_orig = (cv::Mat_<double>(3, 3) << fx_orig, 0, cx_orig,
                                                    0, fy_orig, cy_orig,
                                                    0, 0, 1);
      double k1 = -0.04268357288416628, k2 = 0.0024212974706268172, k3 = -0.000872012773202397, k4 = 0.00012160742067349935;
      ros::param::param<double>("laserMapping/k1", k1, k1);
      ros::param::param<double>("laserMapping/k2", k2, k2);
      ros::param::param<double>("laserMapping/k3", k3, k3);
      ros::param::param<double>("laserMapping/k4", k4, k4);
      cv::Mat cvD_orig = (cv::Mat_<double>(1, 4) << k1, k2, k3, k4);

      cv::Mat cvK_new = (cv::Mat_<double>(3, 3) << colmap_fx, 0, colmap_cx,
                                                   0, colmap_fy, colmap_cy,
                                                   0, 0, 1);

      cv::fisheye::initUndistortRectifyMap(cvK_orig, cvD_orig, cv::Mat::eye(3, 3, CV_64F), cvK_new,
                                          cv::Size(colmap_width, colmap_height), CV_16SC2, undist_map1, undist_map2);
      has_undist_map = true;

      fout_camera << "1 PINHOLE " << colmap_width << " " << colmap_height << " "
          << std::fixed << std::setprecision(6)
          << colmap_fx << " " << colmap_fy << " "
          << colmap_cx << " " << colmap_cy << std::endl;
    }
    fout_camera.close();
  }
...
void VIOManager::dumpDataForColmap()
{
  ...
  if (pinhole_cam != nullptr)
  {
    ...
    cv::imwrite(image_path, img_rgb_undistort, compression_params);
  }
  else if (has_undist_map && !img_full.empty())
  {
    cv::Mat img_undist;
    cv::remap(img_full, img_undist, undist_map1, undist_map2, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
    cv::imwrite(image_path, img_undist, compression_params);
  }
  else
  {
    cv::imwrite(image_path, img_rgb, compression_params);
  }
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  else if (img.cols < img.rows && width > height)
  {
