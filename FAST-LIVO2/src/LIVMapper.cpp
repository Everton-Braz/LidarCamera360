...
{
  std::cout << "\033[1;32m[FAST-LIVO2] savePCD() invoked! Points to save: " 
            << (img_en ? pcl_wait_save->points.size() : pcl_wait_save_intensity->points.size()) 
            << "\033[0m" << std::endl;
  if (pcd_save_en && (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) && pcd_save_interval < 0) 
  {
    ...
    if (img_en)
    {
      ...
      std::cout << "[FAST-LIVO2] Saving raw point cloud to: " << raw_points_dir << " (" << pcl_wait_save->points.size() << " points)..." << std::endl;
      ...
      if(colmap_output_en)
      {
        ...
        fout_points.flush();
        fout_points.close();
        std::cout << GREEN << "COLMAP points3D.txt saved successfully with " << downsampled_cloud->size() << " points!" << RESET << std::endl;
      }
    }
    else
    {      
