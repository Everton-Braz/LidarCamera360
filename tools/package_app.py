...
def main():
    ...
    if not (native/'fastlivo2.exe').is_file():raise SystemExit('Build the native engine first')
    stage=ROOT/'build/package-data';licenses=stage/'licenses';licenses.mkdir(parents=True,exist_ok=True)
    shutil.copy2(ROOT/'FAST-LIVO2/LICENSE',licenses/'FAST-LIVO2.txt')
    for project in ('Sophus','rpg_vikit'):
        for f in (ROOT/'build/deps-src'/project).rglob('*'):
            if f.is_file() and f.name.lower() in ('license','license.txt','copying'):
                shutil.copy2(f,licenses/(project+'-'+str(f.relative_to(ROOT/'build/deps-src'/project)).replace('/','_').replace('\\','_')))
    ...
    cmd=[sys.executable,'-m','PyInstaller','--noconfirm','--clean','--name','RavenCalibrator',
         '--onefile' if a.onefile else '--onedir','--console',
         ...
         '--paths',str(ROOT),'--collect-submodules','rosbags',
         '--collect-all','qfluentwidgets','--collect-all','PyQt6',
         '--add-data',f'{ROOT / "FAST-LIVO2/config"};FAST-LIVO2/config',
         '--add-data',f'{ROOT / "calibracao_rigida_raven_insta360.json"};.',
         '--add-data',f'{stage};.',
         '--add-data',f'{ROOT / "docs/STANDALONE.md"};docs']
    ...
    for f in sorted(native.glob('*')):
        if f.suffix.lower() in ('.exe','.dll'):cmd+=['--add-binary',f'{f};bin']
    spirula=ROOT/'spirula/spirula.exe'
    if spirula.is_file():cmd+=['--add-binary',f'{spirula};spirula']
